...
<template>
  <section class="about section-nopb bg-gray-900 text-white py-20">
    <div class="container max-w-7xl mx-auto px-4 flex flex-col md:flex-row gap-12 items-center">
        ...
        <FadeInOnScroll>
          <div class="section_header max-w-lg mb-12 space-y-4" data-aos="fade-up">
            <p class="text-sm font-semibold uppercase tracking-wider text-sky-400 flex items-center gap-2 max-w-max">
              ...
              {{ t('aboutSectionService.header.subLabel') }}
            </p>
            <h2 class="text-4xl md:text-5xl font-extrabold leading-tight">
              {{ t('aboutSectionService.header.title.part1') }}
              <span class="text-emerald-400">{{ t('aboutSectionService.header.title.highlight') }}</span>
              {{ t('aboutSectionService.header.title.part2') }}
            </h2>
            <p class="text-gray-400 text-base md:text-lg leading-relaxed">
              {{ t('aboutSectionService.header.description') }}
            </p>
          </div>
        </FadeInOnScroll>

        <ul class="achievements flex flex-col space-y-6">
          <li v-for="(item, index) in achievements" :key="index" class="achievements_item">
            <FadeInOnScroll>
              <div class="flex items-center gap-4" :data-aos="'fade-up'" :data-aos-delay="100 + index * 50">
                <svg
                ...
                <span class="label text-gray-300 font-semibold text-lg">{{ t(item.labelKey) }}</span>
              </div>
            </FadeInOnScroll>
          </li>
        </ul>
      ...
      <!-- Media -->
      <div
        class="media md:w-1/2 rounded-lg overflow-hidden shadow-lg"
        data-aos="zoom-in"
...
.achievements_item .advantages-icon {
  min-width: 2.5rem;
}
</style>
